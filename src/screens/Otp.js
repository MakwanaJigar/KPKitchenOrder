...
const Otp = ({ navigation, route }) => {
  ...
  const [remainingSeconds, setRemainingSeconds] = useState(RESEND_TIME);
  ...
  const otpGap = isSmallScreen ? 5 : 8;
    ...
    const loadEmail = async () => {
      try {
        ...
        if (routeEmail && String(routeEmail).trim()) {
          const cleanRouteEmail = String(routeEmail).trim().toLowerCase();

          setEmail(cleanRouteEmail);

          await AsyncStorage.setItem(RESET_EMAIL_STORAGE_KEY, cleanRouteEmail);

          return;
        }

        const storedEmail = await AsyncStorage.getItem(RESET_EMAIL_STORAGE_KEY);

        if (storedEmail) {
          setEmail(storedEmail.trim().toLowerCase());
        }
      } catch (error) {
        console.log('OTP LOAD EMAIL ERROR:', error);
      } finally {
  ...
  const maskedEmail = useMemo(() => {
    if (!email || !email.includes('@')) {
      return email || 'your registered email address';
    }
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      await AsyncStorage.setItem(RESET_OTP_STORAGE_KEY, verificationCode);
      ...
      if (resetToken) {
        ...
        await AsyncStorage.removeItem(RESET_TOKEN_STORAGE_KEY);
      }

      console.log('RESET SESSION STORED');

      console.log('EMAIL:', cleanEmail);

      console.log('OTP:', verificationCode);

      console.log('RESET TOKEN:', resetToken ? 'AVAILABLE' : 'NOT RETURNED');
  ...
  const handleResendOtp = async () => {
    if (remainingSeconds > 0 || resending || loading) {
      return;
    }
    ...
    try {
      ...
      setRemainingSeconds(RESEND_TIME);
    ...
    return (
      <SafeAreaView style={styles.screen}>
        ...
        <View style={styles.screenLoader}>
          ...
          <Text style={styles.screenLoaderText}>Preparing verification...</Text>
        </View>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={styles.screen}>
      ...
      >
        <ScrollView
          ...
          /*
           * Keep buttons and OTP fields
           * usable while keyboard is open.
           */
        ...
        >
          ...
          >
            ...
            <Text numberOfLines={2} style={styles.emailText}>
              {maskedEmail}
            </Text>
            ...
            <TouchableOpacity
              ...
              style={[
                ...
                (loading || resending || !email) && styles.verifyButtonDisabled,
              ]}
            ...
            <View style={styles.helpCard}>
              ...
              <Text style={styles.helpText}>
                The verification code will expire shortly. Never share this code
                with anyone.
              </Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  scrollContent: {
     ...
     * so Verify/Resend sections
     * can move above keyboard.
     */
    paddingBottom: 120,
  },
